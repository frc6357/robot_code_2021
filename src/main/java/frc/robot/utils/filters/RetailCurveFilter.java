...
/**
 * This filter follows a retail curve. The variable coefficient will specify the degree of
 * the retail curve. It must be greater than zero and less than ten. The retail formula
 * follows the formula: f(x) = x*(c/9) + x^5 * (9-c)/9
 */
public class RetailCurveFilter extends Filter
{
    /**
     * The coefficient by which the output is multiplied.
     */
    private double coefficient;

    /**
     * The maximum value that the output can return.
     */
    private double gain;

    /**
     * Constructor which generates a retail curve with the degree of coef (affects
     * curvature)
     * 
     * @param coef
     *            the coefficient of the retail curve (affects curvature)
     */
     ...
     * 
     * @param coef
     *            the coefficient with which to adjust curvature
     * @param gain
     *            the gain which which to adjust max value
     */
    ...
    {
        return gain * (rawAxis * coefficient / 9
            + Math.pow(rawAxis, 5) * (9 - coefficient) / 9);
    }
