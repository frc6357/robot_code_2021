 ...
 */
public class DriveStraightCommandNew extends CommandBase {
     ...
     */
    public DriveStraightCommandNew(SK20Drive subsystem, double distanceTarget) {
    ...
    public void execute() {
        ...
        double absDeltaLeftEncoderDistance = Math.abs(leftEncoderDistance - initialLeftEncoderValue);
        double absDeltaRightEncoderDistance = Math.abs(rightEncoderDistance - initialRightEncoderValue);
        double absDelta = Math.abs(absDeltaLeftEncoderDistance - absDeltaRightEncoderDistance);
        double absDistanceTarget = Math.abs(distanceTarget);
        double driveSpeed = TuningParams.AUTONOMOUS_DRIVE_SPEED * Math.signum(distanceTarget);


        if (absDeltaLeftEncoderDistance >= (absDistanceTarget - TuningParams.AUTONOMOUS_SLOW_DISTANCE_AREA)
                || absDeltaRightEncoderDistance >= (absDistanceTarget - TuningParams.AUTONOMOUS_SLOW_DISTANCE_AREA)) {
        ...
        if (absDelta <= TuningParams.STRAIGHT_DRIVE_OFFSET_TOLERANCE) {
            ...
            if (absDeltaLeftEncoderDistance > absDeltaRightEncoderDistance) {
        ...
        if (absDeltaLeftEncoderDistance >= Math.abs(distanceTarget) || absDeltaRightEncoderDistance >= Math.abs(distanceTarget)) {
